...
function resolveBin(name3) {
  ...
  while (true) {
    if (process.platform === "win32") {
      const exePath = path2.join(dir, "node_modules", ".bin", `${name3}.exe`);
      if (fs.existsSync(exePath))
        return exePath;
      const cmdPath = path2.join(dir, "node_modules", ".bin", `${name3}.cmd`);
      if (fs.existsSync(cmdPath))
        return cmdPath;
    } else {
      const binPath = path2.join(dir, "node_modules", ".bin", name3);
...
function getFileContent(path6) {
  ...
  try {
    return readFileSync3(join2(this.absolutePackageDir, path6)).toString();
  } catch (err) {
    return void 0;
  }
}
      ...
      return {
        ...
        getFileContent: (filePath) => {
          return getFileContent.call(
            {
              absolutePackageDir: isAbsolute ? packagePath : path3.join(process.cwd(), packagePath)
            },
            filePath
          );
        }
...
var getRefineConfig = async (packagePath, isAbsolute) => {
  try {
    ...
    const config2 = __require(path4.join(
      isAbsolute ? packagePath : path4.join(process.cwd(), packagePath),
      "refine.config.js"
    ));
    return config2;
  } catch (error) {
...
`
  ...
  const createdFiles = await Promise.all(
    selectedComponent.files.map(async (file) => {
      var _a2;
      try {
        const srcPath = file.src ? path5.join(selectedPackage.path, file.src) : void 0;
        const destPath = file.dest ? path5.join(process.cwd(), projectPathPrefix, file.dest) : void 0;
        if (!srcPath) {
